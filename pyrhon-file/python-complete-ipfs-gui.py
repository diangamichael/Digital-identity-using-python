...
class StartupWindow:
    def __init__(self, master):
        ...
        label_file_explorer = Label(self.frame,
                                    ...
                                    width=100, height=4,
                                    fg="blue").pack()
        self.button1 = Button(self.frame, text='Upload',
                              width=25, command=self.upload_Hash).pack()
        self.button2 = Button(self.frame, text='getHash',
                              width=25, command=self.get_Hash).pack()
        self.button3 = Button(self.frame, text='updateHash',
                              width=25, command=self.update_Hash).pack()
        self.button4 = Button(self.frame, text='deleteHash',
                              width=25, command=self.delete_Hash).pack()
        self.button5 = Button(self.frame, text='Quit',
                              width=25, command=self.close_windows).pack()
        self.frame.pack()
...
class UploadHash:
    def __init__(self, master):
        ...
        self.quitButton = Button(
            self.frame, text='Open', width=25, command=self.forOpenFileandSubmit).pack()
        self.quitButton = Button(
            self.frame, text='Quit', width=25, command=self.close_windows).pack()
        self.frame.pack()

    def forOpenFileandSubmit(self):
        self.filename = filedialog.askopenfilename(
            initialdir="/home/zero/Desktop", title="Select A File", filetypes=(("all files", "*.*"), ("jpg files", "*.jpg")))
        print(self.filename)
        ...
        self.btn1 = Button(self.frame, text="Submit",
                           width=25, command=self.myClick).pack()
    ...
    def myClick(self):
        try:
            ...
            try:
                ...
                self.tx_hash = contract.functions.addHash(
                    self.hashVal, int(self.elment)).transact()
                self.tx_receipt = web3.eth.waitForTransactionReceipt(
                    self.tx_hash)
                print(
                    f'transaction hash : {self.tx_hash}  transaction receipt : {self.tx_receipt}')
            except:
                print('there is transaction error line: 98')
                exit()
            self.myLabel = Label(
                self.frame, text=f"{self.filename} for hash value :--> {self.hashVal}").pack()
            self.myLabel = Label(
                self.frame, text='file sucessfully uploaded').pack()
            print('Sucsessfully uploaded')
        except:
            print(
                'Please turn on the IPFS Daemon Process if you are using linux run startDeamon.sh')
...
class GetHash:
    def __init__(self, master):
        ...
        label_file_explorer = Label(self.frame,
                                    ...
                                    fg="blue").pack()
        self.quitButton = Button(
            self.frame, text='get', width=25, command=self.get_hash_window).pack()
        self.quitButton = Button(
            self.frame, text='Quit', width=25, command=self.close_windows).pack()
        self.frame.pack()
    ...
    def get_hash_window(self):
        label = Label(
            self.frame, text='Enter your file number for retriving hash : ').pack()
        self.entry = Entry(self.frame, text='Enter IPFS file number2')
        self.entry.pack()
        self.entryBtn = Button(self.frame, text='Get hash',
...
class UpdateHash:
    def __init__(self, master):
        ...
        label_file_explorer = Label(self.frame,
                                    ...
                                    fg="blue").pack()
        self.quitButton = Button(
            self.frame, text='put old hash', width=25, command=self.update_hash_oldHash).pack()
        self.quitButton = Button(
            self.frame, text='Quit', width=25, command=self.close_windows).pack()
        self.frame.pack()
    ...
    def update_hash_oldHash(self):
        ...
        entryBtn1 = Button(self.frame, text='put new hash',
                           command=self.update_hash_newHash).pack()
    ...
    def update_hash_newHash(self):
        ...
        self.entry2.pack()
        self.entryBtn2 = Button(
            self.frame, text='put hash number', command=self.update_hash_oldHashnumber).pack()

    def update_hash_oldHashnumber(self):
        label = Label(
            self.frame, text='Enter your file old hash number : ').pack()
        self.entry3 = Entry(self.frame, text='Enter IPFS file number3')
        self.entry3.pack()
        self.entryBtn3 = Button(
            self.frame, text='update value', command=self.updateValue).pack()
    ...
    def updateValue(self):
        ...
        self.tx_hash = contract.functions.updateHash(
            self.newHash, self.oldHash, int(self.hashNo)).transact()
        self.tx_receipt = web3.eth.waitForTransactionReceipt(self.tx_hash)
        print(
            f'transaction hash : {self.tx_hash}  transaction receipt : {self.tx_receipt}')
        self.myLabel = Label(self.frame, text='file sucessfully update').pack()
...
class DeleteHash:
    def __init__(self, master):
        ...
        self.quitButton = Button(
            self.frame, text='put hash', width=25, command=self.getHash).pack()
        self.quitButton = Button(
            self.frame, text='Quit', width=25, command=self.close_windows).pack()
        self.frame.pack()
    ...
    def getHash(self):
        ...
        self.entry1.pack()
        print(self.entry1.get())
        entryBtn1 = Button(self.frame, text='put hash number',
                           command=self.getNumber).pack()
    ...
    def getNumber(self):
        ...
        self.entry1.pack()
        print(self.entry1.get())
        entryBtn1 = Button(self.frame, text='Delete hash',
                           command=self.hash_del).pack()
    ...
    def hash_del(self):
        ...
        try:
            self.tx_hash = contract.functions.deleteHash(
                self.hashVal, int(self.hashNo)).transact()
            self.tx_receipt = web3.eth.waitForTransactionReceipt(self.tx_hash)
            print(
                f'transaction hash : {self.tx_hash}  transaction receipt : {self.tx_receipt}')
            self.myLabel = Label(
                self.frame, text='file hash sucessfully delete').pack()
